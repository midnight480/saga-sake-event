...
`vercel.json` の `regions` で、サーバーを **sin1（シンガポール）** に決めている。データの置き場所
（Neon）と同じ地域にするため。別の地域にすると、データを取るたびに遠くと往復し、画面の
切り替えがはっきり遅くなる（Issue #60。米国東部とシンガポールで 1 秒以上かかっていた）。

- `regions` を消したり変えたりしない。変えるなら、Neon の地域も同じにすること
- 1 回の要求の中で、データベースへの問い合わせを**順番に**何度も重ねない。並べられるものは
  `Promise.all` で同時に投げる。往復の回数がそのまま待ち時間になる
- 4 秒ごとに呼ばれる処理（`/api/snapshot`）から、外部の API を毎回呼ばない

## 利用規約・プライバシーポリシー

本文は `src/components/LegalDocuments.tsx` の 1 か所だけにある。**中身を変えたら
`src/lib/legal.ts` の `LEGAL_VERSION` を上げる**。上げると、参加者と酒蔵にもう一度同意を求める
（誤字の直しなど、中身が変わらない修正では上げなくてよい）。

外部サービスを足したり、保存先の地域を変えたりしたら、プライバシーポリシーの「外部のサービスの
利用と、情報の保存場所」も直すこと。書かれていない国に情報を送るのは、同意の範囲の外になる。
