...
| --- | --- | --- | --- | --- |
| **Vercel** | サイトの置き場所 | Hobby（$0） | Pro（$20／月〜。使った分は従量） | 小〜中規模なら無料で動く。**商用なら Pro**（下記） |
| **Neon** | データの保存 | Free（$0） | Launch（従量。最低料金なし） | 無料で足りる |
...
| イベントの規模 | かかる費用 | やること |
| --- | --- | --- |
| 主催者・酒蔵・参加者の**合計が 100 人まで**、入場無料など非商用 | **無料** | なし |
| **合計が 100 人を超える** | **ドメイン代だけ**（持っていなければ年 1,500 円ほど） | イベント前に [独自ドメインで本番運用する](docs/独自ドメインで本番運用する.md) |
| 入場料を取るなど、**Vercel の規約で商用にあたる**イベント | **Vercel Pro の $20／月〜** | Vercel を Pro にする（イベントのある月だけでも可） |
| 来場者が**数百人を超え、長時間開いたまま**使われる | Vercel の従量分が加わることがある | 予行演習のあと、Vercel の **Usage** 画面で使った量を確かめる |
...
```
例: 開催 5 時間、出展 30 蔵（画面を開いたまま）、参加者 300 人（3 割の時間だけ開く）

  酒蔵    30 台 × 900 回 × 5 時間         = 約 13.5 万回
  参加者 300 人 × 900 回 × 5 時間 × 0.3   = 約 40.5 万回
                                    合計   約 54 万回
```

Hobby の「月 100 万回」に収まりますが、余裕は 2 倍ほどです。参加者が多い、画面を
開いたままにする人が多い、同じ月にイベントや予行演習を何度も行う、といった場合は
超えることがあります。**予行演習のあとに Vercel の Usage 画面で実際の量を確かめ**、
足りなさそうなら Pro にしてください。
